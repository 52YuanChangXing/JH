...
const elements = {
  ...
  previewFrame: document.getElementById('site-preview'),
...
const sectionLabels = {
  hero: '首页英雄区',
  metrics: '品牌指标',
  featureToggles: '功能组件开关',
  content: '整站配置',
  studioValues: collectionLabels.studioValues,
  ...collectionLabels,
  ...toggleLabels
};
...
function updateAuthUI(isAuthenticated) {
  ...
  if (elements.previewSection) {
    elements.previewSection.classList.toggle('is-hidden', !(isAuthenticated && hasContent));
  }
...
function refreshPreview(reason = '', immediate = false) {
  ...
  const reload = () => {
    try {
      if (elements.previewFrame.contentWindow) {
        elements.previewFrame.contentWindow.location.reload();
      } else {
    ...
    if (reason) {
      setSyncStatus('ready', `${reason} · ${new Date().toLocaleTimeString()}`);
    }
  };
  if (immediate) {
    reload();
...
function initRealtimeBridge() {
  ...
  realtimeSource.addEventListener('open', () => {
    setSyncStatus('ready', `实时联动 · ${new Date().toLocaleTimeString()}`);
  });
  realtimeSource.addEventListener('contentUpdated', event => {
    const data = parseEventData(event);
    reloadContent();
    const label = data.section ? sectionLabels[data.section] || data.section : '站点内容';
    refreshPreview(`${label} 更新`, false);
  });
  realtimeSource.addEventListener('bookingCreated', event => {
    const data = parseEventData(event);
    reloadBookings();
    const label = data.id ? '新预约已同步' : '预约列表更新';
    refreshPreview(label, false);
  });
  realtimeSource.addEventListener('bookingProgressed', event => {
    const data = parseEventData(event);
    reloadBookings();
    const stage = data.stage;
    if (stage && state.content) {
      const found = state.content.timelineStages?.find(item => item.id === stage);
      if (found) {
        refreshPreview(`${found.name} 已同步`, false);
        return;
      }
    }
    refreshPreview('项目进度更新', false);
  });
  realtimeSource.addEventListener('error', () => {
    setSyncStatus('error', '实时连接中断，正在尝试重连…');
  });
}
...
function bindEvents() {
  ...
  if (elements.previewRefresh) {
    elements.previewRefresh.addEventListener('click', () => {
      refreshPreview('手动刷新', true);
    });
  }
